...
// each - so an old backup still restores its cars, and an old copy of the app
// reading a new backup still finds them.
...
const mileage = (): MileageData => ({
  ...
  lease: { ...EMPTY_MILEAGE.lease, startDate: '2026-03-02', startOdometerKm: 62_000 },
...
describe('the backup', () => {
  ...
  it('reads a backup from before mileage was in it as having none, and keeps its housing', () => {
    const back = parseBackup(JSON.stringify({ ...cars(), housing: housing() }))

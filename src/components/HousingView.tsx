  ...
  return (
    <>
      <SituationPanel
        ...
        edited={data.situationUpdatedAt !== ''}
      ...
      {/* The places straight after the ceiling they are measured against:
          they are what gets revisited, the analysis below is read once and
          folds away. */}
...
function SituationPanel({
  ...
  /** whether anyone has typed into it yet */
  edited: boolean
  onChange: (s: HousingSituation) => void
}) {
  // Folded on a desktop too, not only on a phone: open, it was a screen tall
  // above the ceiling it feeds. Open on a first visit, though - the defaults
  // are nobody's income - and after that the device remembers.
  const [expanded, toggle] = useOpen('housing.situation', !edited)
...
function PropertyCard({
  ...
  return (
    <div className={`card car-card${selected ? ' selected' : ''}`}>
      ...
      {/* A row of their own: beside the name, two chips this long left it a
          few letters of width - "Tapanila" one letter a line. */}
...
function HousingTable({
  ...
  // The value rows fold away with their long note: they are the one part of
  // the table that is not a cost, and the part read least often.
  const [projectionOpen, toggleProjection] = useOpen('housing.table.projection')
              ...
              <>
                <tr className="cmp-group">
                  <th colSpan={list.length + 1}>
                    <button
                      className="fold-toggle cmp-group-toggle"
